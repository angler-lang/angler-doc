...
\end{split}
\end{equation*}
\captionof{figure}{Gramática de precedencia de operadores basada en \textcite{parsing-mixfix-operators}}
...
class Grupoid t where
    (<+>) :: t -> t -> t

-- tiene un `forall t:Type .` implícito
instance Grupoid [t] where
    -- concat
    (<+>) = concat
        where
            concat []     bs = bs
...
\begin{anglercode}
closed Grupoid : Type -> Type with
        GrupoidOp : forall a:Type . (a -> a -> a) -> Grupoid a

ListGrupoid : forall t:Type . Grupoid (List t)
ListGrupoid = GrupoidOp concat
    where
        concat : forall t:Type . List t -> List t -> List t
        concat Nil bs = bs
        concat (a :: as) bs = a :: concat as bs

self : forall t:Type . Grupoid t -> t -> t
self (MonoidOp op) a = op a a

self ListGrupoid (1 :: 2 :: Nil)    -- 1 :: 2 :: 1 :: 2 :: Nil
\end{anglercode}

\begin{anglercode}
closed Eq : Type -> Type with
    EqOp : forall t:Type . (t -> t -> Bool) -> Eq t

ListEq : forall t:Type . Eq (List t)
ListEq = EqOp equal
    where
        equal : forall t:Type . List t -> List t -> List t
        equal Nil Nil = True
...
elem _         _ Nil = False
elem (EqOp op) x (y :: ys) = op x y || elem (EqOp op) x ys

elem BoolEq False (False :: True :: Nil)    -- True
\end{anglercode}

La mayor diferencia es que para utilizar estas funciones en nuestro lenguaje, se debe pasar la \emph{instancia} a utilizar explícitamente. De hecho, esta es una manera válida para plantear las clases de tipos (\textcite{scrap-type-classes}), como azúcar sintáctica para tipos y constructores.
\end{designfr}
